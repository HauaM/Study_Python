...
class App:
    def __init__(self, master):
        ...
        #Button
        Button(frame,text='Entry').grid(row=0,column=2)
        ...
        b1=Radiobutton(radio_frame,text='portrait',variable=radio_select,value='P')
        b1.pack(side=LEFT)
        ...
        #Scale
        scale(frame,from_=1,to=10,orient=HORIZONTAL,variable=scale_var).grid(row=2,column=0)
        Label(frame, textvariable=scale_var, font=("Helvetica",36)).grid(row=2,column=1)

        #Message
        message=Message(frame,text='Multiline Message Area')
